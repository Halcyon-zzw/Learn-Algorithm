package offer.offer_1_50.offer_43.impl;
 ...
 * @Version: 1.0
 */
public class CountDigitOneRecursionImpl implements CountDigitOne {
    @Override
    ...
    private int recursionCountDigitOne(int n) {
        ...
        count = hightDigitVal * recursionDigitSumOfOne(digitCount - 1) + recursionCountDigitOne(nextDigitNum);
     ...
     * @return
     */
    private int recursionDigitSumOfOne(int disits) {
        if (disits == 1) {
            return 1;
        }
        return recursionDigitSumOfOne(disits - 1) * 10 + (int)Math.pow(10, disits -1);
    }
