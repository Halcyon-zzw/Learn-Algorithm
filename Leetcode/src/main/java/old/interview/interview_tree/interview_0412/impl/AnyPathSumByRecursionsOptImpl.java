 ...
 * @Version: 1.0
 */
public class AnyPathSumByRecursionsOptImpl implements AnyPathSum {
    ...
    private int findPathSum(TreeNode root, int sum) {
        ...
        sum -= root.val;
        int count = sum == 0 ? 1 : 0;
        return count + findPathSum(root.left, sum) + findPathSum(root.right, sum);
    }

    public static void main(String[] args) {
        TestUtils.test(AnyPathSumByRecursionsOptImpl.class, TreeNode.create(5,4,8,11,-1,13,4,7,2,-1,-1,5,1), 22);
    }
}
